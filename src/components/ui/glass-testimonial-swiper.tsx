...
export const TestimonialStack = ({ testimonials, visibleBehind = 2 }: TestimonialStackProps) => {
    ...
    return (
        <section className="testimonials-stack relative pb-10 w-full max-w-4xl mx-auto h-[500px] flex items-center justify-center">
            {testimonials.map((testimonial, index) => {
                ...
                return (
                    <div
                        ...
                        key={testimonial.id}
                        className="testimonial-card glass-effect backdrop-blur-xl absolute w-full max-w-md bg-card/80 border border-border rounded-xl shadow-lg transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]"
            ...
            <div className="pagination flex gap-2 justify-center absolute bottom-0 left-0 right-0">
                {testimonials.map((_, index) => (
                    <button
                        ...
                        className={`pagination-dot h-2 w-2 rounded-full transition-all duration-300 ${activeIndex === index ? 'bg-primary w-4' : 'bg-muted-foreground/30 hover:bg-muted-foreground/50'}`}
                    />
                ))}
            </div>
        </section>
